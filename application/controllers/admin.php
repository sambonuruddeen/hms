...

{
...

	{
		...
		$page_data['page_name']      = 'view_users';

		$page_data['page_title']     = get_phrase('view_users');
...

	{
		...
		$page_data['page_name']        = 'activity_log';

		$page_data['page_title']       = get_phrase('users_activity_log');

		$this->db->where('user_identifier', '0');

		$this->db->delete('usertracking');

		$this->db->order_by('id', 'DESC');

		$page_data['activity']    = $this->db->get('usertracking')->result_array();

		$this->load->view('index', $page_data);
	}

	/***** All Patients Record Log *****/
	...
	
	{
		...
		$page_data['page_name']        = 'view_patient_log';

		$page_data['page_title']       = get_phrase('patients_record_log');

		$this->db->select('patient_id');
		$this->db->distinct('patient_id');
		//$this->db->order_by('creation_timestamp', 'DESC');

		$page_data['p_log']			   = $this->db->get('prescription')->result_array();

		$this->db->order_by('creation_timestamp','DESC');
		$page_data['list'] = $this->db->get('prescription')->result_array();
...
/***** Single Patient Record *****/
	function single_patient_log($patient_id = '') {

		$page_data['page_name'] = 'single_log';
		$page_data['page_title'] = 'Patient History';

		$this->db->where('patient_id', $patient_id);
		$page_data['patient'] = $this->db->get('patient')->result_array();

		$this->db->order_by('creation_timestamp','DESC');
		$this->db->where('patient_id', $patient_id);
		$page_data['records'] = $this->db->get('prescription')->result_array();
